  ...
  try {
    ...
    for (const deptData of departmentsToCreate) {
      ...
      if (!dept) {
        ...
        results.push(`Created department: ${dept.name}`);
      } else {
        results.push(`Department already exists: ${dept.name}`);
      }
    ...
    for (const progData of programmesToCreate) {
      ...
      if (!prog) {
        ...
        results.push(`Created programme: ${prog.name}`);
      } else {
        results.push(`Programme already exists: ${prog.name}`);
      }
    ...
    for (const blockData of blocksToCreate) {
      ...
      if (!block) {
        ...
        results.push(`Created block: ${block.name}`);
      } else {
        results.push(`Block already exists: ${block.name}`);
      }
    ...
    for (const roomData of roomsToCreate) {
      ...
      if (!room) {
        ...
        results.push(`Created room: ${room.roomNumber}`);
      } else {
        results.push(`Room ${roomData.roomNumber} already exists`);
      }
    }
